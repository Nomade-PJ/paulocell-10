/**
 * Utilitário para reinicializar todas as estatísticas do sistema
 * Limpa todos os dados armazenados mas mantém a estrutura dos relatórios
 */

// Função para reinicializar estatísticas e dados dos relatórios
export function resetAllStatistics() {
  try {
    // Lista de chaves relacionadas a dados que precisam ser resetados
    const keysToReset = [
...
// Função para zerar apenas estatísticas visuais mantendo dados reais
export function resetVisualStatistics() {
  try {
    // Remover apenas dados de visualização estatística
    localStorage.removeItem('pauloCell_chartData');
    localStorage.removeItem('pauloCell_reportData');
    localStorage.removeItem('pauloCell_dashboard_stats');
