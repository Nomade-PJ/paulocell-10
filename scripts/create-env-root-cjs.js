...
// Este script usa CommonJS para compatibilidade máxima e pode ser executado de qualquer diretório
const fs = require('fs');
...
function createEnvProduction() {
  ...
  if (!fs.existsSync(projectDir)) {
    ...
    // Tentar encontrar o diretório do projeto em outros locais comuns na Hostinger
    const possibleDirs = [
      '/var/www/html',
      '/var/www',
      '/home/u123456789/public_html',
      '/home/u123456789/domains/seudominio.com/public_html',
      process.cwd() // Usar o diretório atual como último recurso
    ];
    
    for (const dir of possibleDirs) {
      if (fs.existsSync(dir)) {
        // Verificar se parece ser o diretório do projeto (contém package.json ou server.js)
        if (fs.existsSync(path.join(dir, 'package.json')) || 
            fs.existsSync(path.join(dir, 'server.js'))) {
          projectDir = dir;
          console.log(`Projeto encontrado em: ${projectDir}`);
          break;
        } else {
          console.log(`Diretório ${dir} existe, mas não parece ser o projeto Paulo Cell.`);
          // Verificar subdiretórios comuns
          const subDirs = ['paulocell', 'paulocell-10', 'public_html'];
          for (const sub of subDirs) {
            const fullPath = path.join(dir, sub);
            if (fs.existsSync(fullPath) && 
                (fs.existsSync(path.join(fullPath, 'package.json')) || 
                 fs.existsSync(path.join(fullPath, 'server.js')))) {
              projectDir = fullPath;
              console.log(`Projeto encontrado em subdiretório: ${projectDir}`);
              break;
            }
          }
        }
